...
impl CrudOperations<ClickhouseTransmit> for ClickhouseTransmitBiz {
    ...
    async fn update(&self, id: u64, item: ClickhouseTransmit) -> Result<ClickhouseTransmit, Error> {
        let mut updates = vec![];
    ...
    async fn list(&self, filters: Vec<Filter>) -> Result<Vec<ClickhouseTransmit>, Error> {
        log::info!(
    ...
    async fn by_id(&self, id: u64) -> Result<ClickhouseTransmit, Error> {
        let result =
