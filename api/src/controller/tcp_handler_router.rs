...
pub async fn create_tcp_handler(
    ...
    match tcp_handler_api.create(data.into_inner()).await {
        Ok(u) => {
            ...
            tcp_handler_api.setRedis(&u);
            tcp_handler_api.set_auth(&u);
...
pub async fn update_tcp_handler(
    ...
    let x = tcp_handler_api.by_id(data.id.unwrap()).await;
    ...
    match x {
        Ok(mut u) => {
            ...
            let result = tcp_handler_api.update(u.id.unwrap(), u).await;
            match result {
                Ok(u2) => {
                    let success_json = json!({
                        "code": 20000,
                    ...
                    tcp_handler_api.setRedis(&u2);
                    tcp_handler_api.set_auth(&u2);
                    let string = calc_bucket_name(
                        config
                            .influx_config
                            .clone()
...
pub async fn by_id_tcp_handler(
    ...
    let result = tcp_handler_api.by_id(id).await;
    match result {
        Ok(u) => {
            let success_json = json!({
                        "code": 20000,
...
#[get("/TcpHandler/page?<page>&<page_size>&<name>")]
...
pub async fn delete_tcp_handler(
    ...
    let result = tcp_handler_api.delete(id).await;
    match result {
        Ok(o) => {
            let success_json = json!({
                "code": 20000,
